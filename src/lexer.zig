...
pub const Tokenizer = struct {
    ...
    /// An eof token will always be returned _sign the end.
...
test "keywords" {
    try testTokenize("test not else", &.{ .keyword_test, .keyword_not, .keyword_else });
}

test "line comment followed by return empty braces" {
    try testTokenize(
        \\// line comment
        \\return {}
        \\
...
test "int array and method call" {
    try testTokenize(
        \\i32[]
        \\hello.world()
    , &.{
...
test "range literals" {
    try testTokenize("0..9", &.{ .number_literal, .ellipsis, .number_literal });
    try testTokenize("'0'..'9'", &.{ .char_literal, .ellipsis, .char_literal });
    try testTokenize("0x00..0x09", &.{ .number_literal, .ellipsis, .number_literal });
    try testTokenize("0b00..0b11", &.{ .number_literal, .ellipsis, .number_literal });
    try testTokenize("0o00..0o11", &.{ .number_literal, .ellipsis, .number_literal });
}
...
test "number literals decimal" {
    ...
    try testTokenize("1..", &.{ .number_literal, .ellipsis });
